...
## La differenza tra trattamentoCartelleValeNetrw.sh e trattamentoCartelleClassic.sh
## sta nel fatto che il primo tratta le cartelle per ValeNetrw, il II come
## solitamente da input. Quindi trattamentoCartelleCalssic.sh è l'equivalente
## di trattamentoCartelle.sh degli altri software.

cp /tmp/backemerg-fullName /tmp/backemerg-fullNameBackup

cat /tmp/backemerg-fullName | sed 's/\/$//g' > /tmp/backemerg-fullNameBackupLessSlash


cat /tmp/backemerg-fullNameBackupLessSlash | sed 's/\// /g' > /tmp/backemerg-sostSlashBlank
...
cat /tmp/backemerg-fullNameBackupLessSlash  > /tmp/backemerg-nomeCartellaIsolata
...
cat /tmp/backemerg-fullNameBackupLessSlash |sed 's/.*\///' > /tmp/backemerg-nomeCartellaIsolata
# testing
#echo " "
...
# cat /tmp/backemerg-fullNameBackupLessSlash |sed 's/\/.*//' > /tmp/backemerg-percorsoIsolato
...
#cat /tmp/backemerg-fullNameBackupLessSlash  |sed 's/'$nomeFile'.*//'  | sed 's/\/$//g' > /tmp/backemerg-percorsoIsolato
...
cat /tmp/backemerg-fullNameBackupLessSlash | cut -d/ -f1,"$numberOfColumn" > /tmp/backemerg-percorsoIsolato
