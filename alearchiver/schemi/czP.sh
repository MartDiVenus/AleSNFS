...
then
	...
	echo "Digita la password per cifrare"

	read -s Password

else
	
	read -p "Digita la password per cifrare
" Password
...
if [ -f /tmp/alearchiver-cimiceFolderOrFile-cartella ]; then
	...
	zip -P $Password -r $nomeCartella.zip $nomeCartella

	else

	zip -P $Password -r $nomeFileSenzaEstensione.zip $nomeFile
...
if [ -f /tmp/alearchiver-cimiceFolderOrFile-cartella ]; then
	...
	zip -P $Password -r $destPathManuale/$nomeCartella.zip $nomeCartella

	else

	zip -P $Password -r $destPathManuale/$nomeFileSenzaEstensione.zip $nomeFile
